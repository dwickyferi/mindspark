...
/**
 * Normalize YouTube URL to ensure it's a proper URL format
 */
export function normalizeYouTubeUrl(input: string): string {
  ...
  // If it's just a video ID, convert to youtu.be format
  const videoId = extractVideoId(input);
  if (videoId) {
    return `https://youtu.be/${videoId}`;
  }
  
  // If all else fails, return the input (shouldn't happen but safety net)
  return input;
}
  ...
  static async getVideoTranscript(url: string): Promise<{ videoInfo: YouTubeVideoInfo; transcript: string }> {
    ...
    try {
      ...
      const videoInfo: YouTubeVideoInfo = {
        ...
        duration: Math.max(metadata.length || metadata.duration || 1, 1), // Ensure duration is at least 1
        url: normalizeYouTubeUrl(metadata.source || url || `https://youtu.be/${videoId}`), // Ensure proper URL format
      };
  ...
  static async getVideoMetadata(url: string): Promise<YouTubeVideoInfo> {
    ...
    try {
      ...
      const basicInfo: YouTubeVideoInfo = {
        ...
        duration: 1, // Set minimum duration to 1 for validation
        url: `https://youtu.be/${videoId}`, // Ensure proper URL format
      };
      ...
      try {
        ...
        if (docs && docs.length > 0) {
          ...
          return {
            ...
            duration: Math.max(metadata.length || metadata.duration || basicInfo.duration, 1), // Ensure duration is at least 1
            url: normalizeYouTubeUrl(metadata.source || basicInfo.url), // Use normalized URL
          };
        }
      } catch (metadataError) {
